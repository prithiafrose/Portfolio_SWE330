export const personal = {
  name: "Prithi Afrose",
  firstName: "Prithi",
  role: "Software Engineering Student",
  university: "Shahjalal University of Science and Technology",
  location: "Sylhet, Bangladesh",
  email: "prithiafrose@gmail.com",
  phone: "+880 1747 863344",
  phoneRaw: "+8801747863344",
  resume: "/images/cv.pdf",
  profileImage: "/images/profile.jpg",
  logo: "/images/logo.png",
  avatarAlt: "Portrait of Prithi Afrose",
  availability: "Open to Internship",
  headline:
    "I'm a Software Engineering student at Shahjalal University of Science and Technology, passionate about turning ideas into functional, user-friendly software — from responsive web apps and mobile solutions to playful game projects. I enjoy learning new technologies and I'm aiming for a career as a Software Engineer.",
  roles: [
    "Software Engineering Student",
    "Aspiring Software Engineer",
    "Web & Mobile App Developer",
    "Game Development Enthusiast",
  ],
};

export const socials = [
  {
    id: "github",
    label: "GitHub",
    href: "https://github.com/prithiafrose",
    icon: "github",
  },
  {
    id: "facebook",
    label: "Facebook",
    href: "https://www.facebook.com/share/14NHTLLkyXZ/",
    icon: "facebook",
  },
  {
    id: "instagram",
    label: "Instagram",
    href: "https://www.instagram.com/prithi_afroz?igsh=MXNidDZyeG5qbzFmdw==",
    icon: "instagram",
  },
  {
    id: "snapchat",
    label: "Snapchat",
    href: "https://www.snapchat.com/add/prithiafrose",
    icon: "snapchat",
  },
];

export const interests = ["Web Apps", "Mobile Apps", "Game Development"];

export const navLinks = [
  { label: "Home", href: "#top" },
  { label: "About", href: "#about" },
  { label: "Skills", href: "#skills" },
  { label: "Projects", href: "#projects" },
  { label: "Experience", href: "#experience" },
  { label: "Contact", href: "#contact" },
];

export const stats = [
  { label: "CGPA", value: "3.73", sub: "B.Sc. Software Engineering" },
  { label: "Projects Completed", value: "6+", sub: "Web, mobile & games" },
  { label: "Technologies", value: "15+", sub: "Across every layer" },
  { label: "Internship Ready", value: "Yes", sub: "Available now" },
];

export const aboutHighlights = [
  {
    icon: "graduation",
    label: "Currently studying",
    value: "Software Engineering @ SUST",
  },
  {
    icon: "code",
    label: "Development focus",
    value: "Web & Mobile Applications",
  },
  {
    icon: "lightbulb",
    label: "Learning mindset",
    value: "Always exploring new technologies",
  },
  {
    icon: "target",
    label: "Career goal",
    value: "Software Engineer",
  },
];

export const experience = [
  {
    date: "2023 — Current",
    title: "Post-Graduation",
    org: "Shahjalal University of Science and Technology",
    kind: "education",
  },
  {
    date: "2024",
    title: "National Case Competition — Mindmaster 2.0",
    org: "Participant",
    kind: "competition",
  },
  {
    date: "2019",
    title: "Math Olympiad",
    org: "Participant",
    kind: "competition",
  },
  {
    date: "2016",
    title: "National Debate Competition",
    org: "Participant",
    kind: "competition",
  },
];

export const education = [
  {
    date: "2023 — Present",
    title: "B.Sc. in Software Engineering",
    org: "Shahjalal University of Science and Technology",
  },
  {
    date: "2020 — 2022",
    title: "Higher Secondary Certificate (HSC)",
    org: "Dinajpur Govt. College",
  },
  {
    date: "2011 — 2019",
    title: "Secondary School Certificate (SSC)",
    org: "Dinajpur Govt. Girls' High School",
  },
];

export const whatIDo = [
  {
    icon: "code",
    title: "Web Development",
    description:
      "Creating user-friendly, responsive, and visually appealing websites that help businesses establish a strong online presence.",
    link: "https://github.com/prithiafrose/project-111",
  },
  {
    icon: "smartphone",
    title: "App Development",
    description:
      "Developing custom mobile apps for Android and iOS, making services and products easy to access on the go.",
    link: "https://github.com/prithiafrose/MyprojectElevatex",
  },
  {
    icon: "gamepad",
    title: "Game Development",
    description:
      "Designing creative and engaging games using modern technology to provide entertainment that keeps users hooked.",
    link: "https://github.com/prithiafrose/BlackJack",
  },
];

export const skillCategories = [
  {
    id: "frontend",
    label: "Frontend",
    icon: "monitor",
    description: "Building responsive, accessible, user-friendly interfaces.",
    skills: [
      { name: "HTML", level: 92 },
      { name: "CSS", level: 88 },
      { name: "JavaScript", level: 78 },
      { name: "Responsive Design", level: 90 },
    ],
  },
  {
    id: "backend",
    label: "Backend",
    icon: "server",
    description: "Designing APIs and server-side logic.",
    skills: [
      { name: "Node.js", level: 70 },
      { name: "REST APIs", level: 72 },
      { name: "Authentication", level: 65 },
    ],
  },
  {
    id: "database",
    label: "Database",
    icon: "database",
    description: "Modeling and querying relational data.",
    skills: [
      { name: "PostgreSQL", level: 68 },
      { name: "Prisma ORM", level: 66 },
      { name: "Firebase", level: 74 },
    ],
  },
  {
    id: "ai",
    label: "AI/ML",
    icon: "brain",
    description: "Exploring intelligent features and models.",
    skills: [
      { name: "AI Integration", level: 62 },
      { name: "Prompt Engineering", level: 68 },
      { name: "Machine Learning Basics", level: 55 },
    ],
  },
  {
    id: "tools",
    label: "Tools",
    icon: "wrench",
    description: "Version control and project collaboration.",
    skills: [
      { name: "Git", level: 82 },
      { name: "GitHub", level: 84 },
      { name: "Game Logic & Design", level: 70 },
    ],
  },
];

export const marqueeTechnologies = [
  "React",
  "Node.js",
  "PostgreSQL",
  "Prisma",
  "Firebase",
  "HTML",
  "CSS",
  "JavaScript",
  "Tailwind CSS",
  "REST APIs",
  "Git",
  "GitHub",
  "Android",
  "iOS",
  "AI/ML",
];

export const learningChips = ["AI/ML", "Backend Architecture", "Cyber Security"];

export const metrics = [
  { value: "3+", label: "Full Stack Projects" },
  { value: "6+", label: "Total Projects Built" },
  { value: "4", label: "Primary Languages" },
];

export const projectFilters = [
  { id: "all", label: "All" },
  { id: "web", label: "Web Apps" },
  { id: "app", label: "Mobile Apps" },
  { id: "game", label: "Games" },
];

export const projects = [
  {
    id: "educore",
    title: "EduCore",
    category: "web",
    featured: true,
    badge: "Featured",
    image: "/images/work-1.jpg",
    description:
      "A full-stack education platform with a responsive React interface, secure JWT + bcrypt authentication, and PostgreSQL data modeled with Prisma ORM.",
    features: [],
    tech: ["React", "Node.js", "Express", "PostgreSQL", "Prisma ORM", "JWT + bcrypt"],
    github: "https://github.com/prithiafrose/EduCore",
    demo: "https://edu-core-prap-ai.vercel.app/",
  },
  {
    id: "prepai",
    title: "PrepAI",
    category: "web",
    featured: true,
    badge: "Featured",
    image: "/images/work-2.jpg",
    description:
      "An AI-powered preparation app — React + Vite frontend with a backend-only OpenAI integration, JWT-secured auth, and PostgreSQL via Prisma.",
    features: [],
    tech: ["React", "Node.js", "OpenAI API", "PostgreSQL", "Prisma ORM", "JWT + bcrypt"],
    github: "https://github.com/prithiafrose/PrepAI",
    demo: "https://prep-ai-client.vercel.app/",
  },
  {
    id: "careerbridge",
    title: "CareerBridge",
    category: "web",
    featured: true,
    badge: "Featured",
    image: "/images/work-3.jpg",
    description:
      "A career connection platform with an HTML/CSS/JS frontend, Node.js + Express backend, and an integrated chatbot assistant.",
    features: [],
    tech: ["HTML", "CSS", "JavaScript", "Node.js", "Express", "Chatbot"],
    github: "https://github.com/prithiafrose/CareerBridge",
    demo: "https://careerbridge-frontend-orpin.vercel.app/",
  },
  {
    id: "shopping",
    title: "Shopping Website",
    category: "web",
    featured: false,
    image: "/images/work-1.jpg",
    description:
      "An online vegetable store where customers can browse products, compare prices, and buy directly from the website.",
    features: [
      "Browse vegetables & pricing",
      "Buy directly from the website",
      "Responsive, user-friendly layout",
    ],
    tech: ["HTML", "CSS", "Responsive Design"],
    github: "https://github.com/prithiafrose/shop",
    demo: null,
  },
  {
    id: "jobapp",
    title: "Job Searching App",
    category: "app",
    featured: false,
    image: "/images/work-2.jpg",
    description:
      "A mobile app that helps people find their desired job based on their skills and qualifications.",
    features: [
      "Search jobs by skills & qualifications",
      "Built for Android & iOS",
    ],
    tech: ["Firebase", "Android", "iOS"],
    github: "https://github.com/prithiafrose/MyprojectElevatex",
    demo: null,
  },
  {
    id: "snake",
    title: "Snake Game",
    category: "game",
    featured: false,
    image: "/images/work-3.jpg",
    description: "A classic snake game remake designed for thrilling, addictive gameplay.",
    features: ["Classic snake gameplay", "Engaging difficulty curve"],
    tech: ["Game Development"],
    github: "https://github.com/prithiafrose/SNAKE_GAME-2.0",
    demo: null,
  },
];

export const cta = {
  eyebrow: "Let's build something together",
  heading: "Open to internship and entry-level opportunities",
  body: "I'm currently looking for a Software Engineering internship where I can contribute, learn fast, and grow with a great team.",
};